...
function CryptoList() {
  ...
  const deleteData = async (categoryid) => {
    try {
      dispatch(toggle_loading(true));
      await http_instanse_level_2.post('/api/marketpuls/addcryptoitem', {
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
  ...
  return (
    <div className="flex flex-col px-8 py-8">
      <div className="flex items-center justify-between">
        ...
        >
          <Link to="/admin-panel/lessons/market-pulse/crypto/add">New</Link>
        </button>
      </div>
      <MAterialTable
        rows={cryptoData} // Set rows to fetched data
        columns={[
          {
            header: 'Title',
                ...
                <div className="flex items-center">
                  ...
                  <BorderedButtonPrimary
                    ...
                    onClick={() => {
                      ...
                      navigate(
                        `/admin-panel/lessons/market-pulse/crypto/${row.original.id}`
                      );
                    }}
