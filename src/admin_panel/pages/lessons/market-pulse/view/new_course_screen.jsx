...
import { show_message, toggle_loading } from "../../../../../redux/features/generalSlice";
...
function MarketPulseNewCourseScreen() {
  ...
  const publish = async () => {
    ...
    try {
      dispatch(toggle_loading(true));
      await http_instanse_level_2.post("/api/marketpuls/addforexitem", JSON.stringify(body));
  ...
  return (
    <div className="flex flex-col px-8 py-8">
      <h1 className="font-semibold text-2xl text-white mb-4">Create New Course</h1>
      <div className="space-y-[10px]">
        ...
        <CategoryList categoryId={data?.categoryid} onCategoryChange={setCategoryId} />
        <CourseLevelType index={data?.courseleveltypeId} onLevelChange={setCourseLevelType} />
        <CourseVisibility isVisible={data?.isvisible} onVisibilityChange={setVisibility} />

        <div className="rounded-sm bg-white p-[30px]">
          <button className="px-[12px] py-[8px] rounded-sm text-white bg-primary" onClick={publish}>
            Publish
          </button>
        </div>
      </div>
