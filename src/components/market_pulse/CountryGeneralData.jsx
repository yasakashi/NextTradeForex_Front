...
function CountryGeneralData({ flexibleBlocklist, onCountryDataChange }) {
  ...
  const addRow = () => {
    if (countries && pairsthatcorrelate && highslows && pairtype && dailyavrage && mainTitle) {
      setData([
        ...data,
        {
          countries,
  ...
  return (
    <div>
      <CustomTextField value={mainTitle} onChange={setMainTitle} helper_text="Main title" helper_text_up_position />
      <div className="grid grid-cols-4 gap-3 mt-[10px]">
        <CustomTextField value={countries} onChange={setCountrName} helper_text="COUNTRIES" helper_text_up_position />
        <CustomTextField
          value={pairsthatcorrelate}
          onChange={setPairsthatcorrelate}
          helper_text="PAIRS THAT CORRELATE:"
          helper_text_up_position
        />
        <CustomTextField
          value={highslows}
          onChange={setHighslows}
          helper_text="2022 HIGHS & LOWS"
          helper_text_up_position
        />
        <CustomTextField value={pairtype} onChange={setPairtype} helper_text="	PAIR TYPE:" helper_text_up_position />
        <CustomTextField
          value={dailyavrage}
          onChange={setDailyavrage}
      ...
      <div className="mt-4">
        {data.map((item, index) => (
          <div key={index} className="flex items-center justify-between border-b py-2">
            {item.mainTitle}
            <span>
              {item.countries} - {item.pairsthatcorrelate} - {item.highslows} - {item.pairtype} - {item.dailyavrage}
            </span>
