...
import {
  getCryptoCurrencies,
  getCryptoItem,
  getCryptoRelatedContent,
} from '../../../pages/market_pulse/api';
import SearchBox from '../Searchbox';
import LoadingSpinner from '../../Loading';

export default function CryptoStory({ selectedSubCategory }) {
  ...
  const [CryptoItem, setCryptoItem] = useState(false);

  const getRelatedSources = async (id) => {
    try {
      if (id) {
        const res = await getCryptoRelatedContent(id);
  ...
  const getCurrencies = async () => {
    ...
    try {
      if (selectedSubCategory) {
        ...
        setCryptoItem(true);
        const res = await getCryptoCurrencies(selectedSubCategory);
  ...
  const fetchCryptoItem = async (id) => {
    try {
      if (id) {
        setCryptoItem(true);
        const res = await getCryptoItem({
          categoryId: id,
          id: null,
        });
        setCryptoItem(false);
        ...
        <div className="w-1/3 h-screen  bg-primary p-5 z-10 relative right-0">
          ...
          {currencies?.length > 0 ? (
            <SearchBox
              ...
              fetchCryptoItem={fetchCryptoItem}
              query={query}
            />
          ) : (
