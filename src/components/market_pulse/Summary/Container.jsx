...
const TechnicalTab = ({ script }) => {
  ...
  useEffect(() => {
    ...
    if (script.includes("<iframe")) {
      ...
      containerRef.current.innerHTML = tempContainer.innerHTML.replace(scriptElement.outerHTML, "");
    ...
    <div className="bg-blue-dark border-[1px] border-gold-light_400 text-white p-4">
          ...
          tabs.map((tab) => (
            <button
              ...
              className={`text-sm font-bold py-2 px-4 ${
                activeTab === tab.id ? "bg-gold-light_400 rounded-t-lg" : "border-b border-gold-light_400"
              }`}
            >
              {tab.maintitle}
            </button>
      ...
      <div className="flex gap-3">
        <div className="flex-1 grid grid-cols-3 gap-4 bg-blue-light p-4 rounded-r-lg">
          {activeTabData?.script && <TechnicalTab script={activeTabData.script} />}
        </div>
      </div>
    </div>
  );
