...
import { edit_course_api } from "../service/edit_course_api";
...
const NewCourse = () => {
  ...
  React.useEffect(() => {
    if (Object.keys(course_data).length === 0) return navigate("/user-profile");
  });
  return (
    <motion.div
      initial={{ opacity: 0 }}
    ...
    >
      <ScreenElements
        ...
        edit_title={course_data.coursename || "Edit "}
        set_course_items={set_items_list}
      />
      <Formik
        enableReinitialize
        initialValues={init_values(course_data)}
        onSubmit={async (values, props) => {
          // console.log({ values });
          try {
            let course = await edit_course_api({
              ...values, 
              ...
              courseleveltypeId: values.coursetypeId,
            });
            if (!values.file) return;
            let form_data = new FormData();
            form_data.append("coverPicture", values.file);
            form_data.append("id", course_data.id);
                      ...
                      <Reorder.Item key={item?.title} value={item}>
                        ...
                        >
                          <Component
                            ...
                            category_id={formik.values.grouptypeId}
                            /**
                            ...
                            course_tags={formik.values.coursetags}
                            /**
                            ...
                            course_type_id={formik.values.coursetypeId}
                            /**
...
const init_values = (initialValues?: new_course_props): new_course_props => {
  
  
    return {
    coursename: initialValues?.coursename || "",
    materialsincluded: initialValues?.materialsincluded || "",
    targetedaudience: initialValues?.targetedaudience || "",
    total_course_duration_hours:
      initialValues?.total_course_duration_hours || "",
    total_course_duration_minutes:
      initialValues?.total_course_duration_minutes || "",
    whatlearn: initialValues?.whatlearn || "",
    issitecourse: initialValues?.issitecourse || false,
    coursetypeId: initialValues?.coursetypeId || 0,
    allowdownload: initialValues?.allowdownload || false,
    allowQA: initialValues?.allowQA || false,
    coursecoverimage: initialValues?.coursecoverimage || "",
    coursedescription: initialValues?.coursedescription || "",
    coursedurationtime: initialValues?.coursedurationtime || 0,
    courseintrofile: initialValues?.courseintrofile || "",
    courseintrofilecontenttype: initialValues?.courseintrofilecontenttype || "",
    courseintrofilefileextention:
      initialValues?.courseintrofilefileextention || "",
    courseintrofileurl: initialValues?.courseintrofileurl || "",
    courseleveltypeId: initialValues?.courseleveltypeId || "",
    courseprice: initialValues?.courseprice || 0,
    coursetags: initialValues?.coursetags || "",
    enddate: initialValues?.enddate || new Date(),
    startdate: initialValues?.startdate || new Date(),
    grouptypeId: initialValues?.grouptypeId || 0,
    isadminaccepted: initialValues?.isadminaccepted || false,
    isprelesson: initialValues?.isprelesson || false,
    lessoncount: initialValues?.lessoncount || 0,
    registerdatetime: initialValues?.registerdatetime || new Date(),
    requirementsinstructions: initialValues?.requirementsinstructions || "",
  };
};
