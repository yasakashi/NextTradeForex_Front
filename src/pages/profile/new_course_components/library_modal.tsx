...
const LibraryModal = ({
  ...
  has_side_bar_action: boolean;
  open: boolean;
  set_open: React.Dispatch<React.SetStateAction<boolean>>;
  title?: string;
  onSave?: (val: any) => void;
  file?: File | null;
  set_file?: (file: any) => void;
  accept_file: "Video" | "Image" | "PDF";
}) => {
  const [current_tab, set_current_tab] = React.useState<
    "Media Library" | "Upload files"
  >(`Upload files`);
  // let [file, set_file] = React.useState<File | null>(null);

  const [open_details, set_open_details] = React.useState(false);
  return (
    <ModalLayout
      onClose={() => {
        set_open(false);
      ...
      open={open}
    >
      <div className="flex" style={{ width: "70vw", height: "85vh" }}>
        {has_side_bar_action && (
...
const Library = ({
  ...
  open?: boolean;
  set_details: React.Dispatch<React.SetStateAction<boolean>>;
}) => {
  const [item_details, set_item_details] = React.useState<null | any>(null);
  const [show_copy_tooltip, set_show_copy_tooltip] = React.useState<null | any>(
    null
  );
            ...
            >
              {new Array(100).toSorted().map((item, i) => (
                <div key={i} className="relative">
                  ...
                  <motion.img
                    src={imgTest as unknown as string}
                    alt=""
                    width={160}
